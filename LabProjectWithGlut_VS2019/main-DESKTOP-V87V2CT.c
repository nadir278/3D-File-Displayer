//  נדיר יעקב Nadir Yaakov		נדב כהן Nadav Cohen
...
void FaceProcessing(Vertex* v1, Vertex* v2, Vertex* v3, GLfloat FaceColor[3])
{
	int i, j;
	float m;
	GLint rectminx, rectmaxx, rectminy, rectmaxy, pixel[2], vec1[3], vec2[3], vec3[3];
	GLfloat alpha, beta, gama, yinter, dx, dy, linex, liney;
	//GLfloat crossv[3], vecbmina[3], veccmina[3], area, vecsize;
	...
	if (GlobalGuiParamsForYou.DisplayType == FACE_VERTEXES)
	{
		...
		//////////////////////////////////////////////////////////////////////////////////
		//getting min x of rect
		rectminx = v1->pointScreen[0];
		if (v2->pointScreen[0] < rectminx)
			rectminx = v2->pointScreen[0];
		if (v3->pointScreen[0] < rectminx)
			rectminx = v3->pointScreen[0];

		//getting max x of rect
		rectmaxx = v1->pointScreen[0];
		if (v2->pointScreen[0] > rectmaxx)
			rectmaxx = v2->pointScreen[0];
		if (v3->pointScreen[0] > rectmaxx)
			rectmaxx = v3->pointScreen[0];

		//getting min y of rect
		rectminy = v1->pointScreen[1];
		if (v2->pointScreen[1] < rectminy)
			rectminy = v2->pointScreen[1];
		if (v3->pointScreen[1] < rectminy)
			rectminy = v3->pointScreen[1];

		//getting max y of rect
		rectmaxy = v1->pointScreen[1];
		if (v2->pointScreen[1] > rectmaxy)
			rectmaxy = v2->pointScreen[1];
		if (v3->pointScreen[1] > rectmaxy)
			rectmaxy = v3->pointScreen[1];

		for ( i = rectminx; i <=rectmaxx ; i++)
		{
			pixel[0] = i;
			for (j = rectminy; j <= rectmaxy; j++)
			{
				pixel[1] = j;
				/*
				vecbmina[0] = v2->pointScreen[0] - v1->pointScreen[0];
				vecbmina[1] = v2->pointScreen[1] - v1->pointScreen[1];
				vecbmina[2] = v2->pointScreen[2] - v1->pointScreen[2];
				
				vecbmina[0] = v3->pointScreen[0] - v1->pointScreen[0];
				vecbmina[1] = v3->pointScreen[1] - v1->pointScreen[1];
				vecbmina[2] = v3->pointScreen[2] - v1->pointScreen[2];
				
				V3cross(crossv, vecbmina, veccmina);
				vecsize = sqrt(pow(crossv[0], 2) + pow(crossv[1], 2) + pow(crossv[2], 2);
				area = vecsize / 2;
				*/
				
				//calculate alpha
				dx = fabs((v1->pointScreen[0] - v2->pointScreen[0]));
				dy = fabs((v1->pointScreen[1] - v2->pointScreen[1]));
				m = dy / dx;
				yinter = v1->pointScreen[1] - (m * v1->pointScreen[0]);
				linex = (-m * i) + j - yinter;
				liney = (-m * v3->pointScreen[0]) + v3->pointScreen[1] - yinter;
				alpha = linex / liney;

				//calculate beta
				dx = abs((v1->pointScreen[0] - v3->pointScreen[0]));
				dy = abs((v1->pointScreen[1] - v3->pointScreen[1]));
				m = dy / dx;
				yinter = v1->pointScreen[1] - (m * v1->pointScreen[0]);
				linex = (-m * i) + j - yinter;
				liney = (-m * v2->pointScreen[0]) + v2->pointScreen[1] - yinter;
				beta = linex / liney;

				//calculate gama
				dx = abs((v2->pointScreen[0] - v3->pointScreen[0]));
				dy = abs((v2->pointScreen[1] - v3->pointScreen[1]));
				m = dy / dx;
				yinter = v2->pointScreen[1] - (m * v2->pointScreen[0]);
				linex = (-m * i) + j - yinter;
				liney = (-m * v1->pointScreen[0]) + v1->pointScreen[1] - yinter;
				gama = linex / liney;
